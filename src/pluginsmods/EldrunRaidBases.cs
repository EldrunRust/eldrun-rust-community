...
using static Oxide.Plugins.EldrunRaidBasesExtensionMethods.ExtensionMethods;

namespace Oxide.Plugins
{
    [Info("EldrunRaidBases", "Eldrun", "36187")]
    [Description("Create fully automated raidable bases with npcs.")]
    public class EldrunRaidBases : RustPlugin
    {
        [PluginReference]
        Plugin
        ...
        private new const string Name = "EldrunRaidBases";
        ...
        public class RandomBase
        {
            ...
            public EldrunRaidBases Instance;
        ...
        public class BaseProfile
        {
            ...
            public BaseProfile(EldrunRaidBases instance)
            {
                Instance = instance;
                Options.AdditionalBases = new();
            }

            public BaseProfile(EldrunRaidBases instance, BuildingOptions options, string name)
            {
                Instance = instance;
                Options = options;
                ProfileName = name;
        ...
        {
            internal EldrunRaidBases Instance;
        ...
        public class QueueController
        {
            ...
            public class DebugMessages
            {
                ...
                internal EldrunRaidBases _instance;
            ...
            public QueueController(EldrunRaidBases instance)
            {
            ...
            {
                if (!spq.spawns.IsCustomSpawn && !SpawnsController.TopologyChecks(spq.options.Biomes, v, spq.protectionRadius, railRadius, out var topology))
                {
        ...
        public class AutomatedController
        {
            ...
            public AutomatedController(EldrunRaidBases instance, bool a, bool b)
            {
        ...
        public class BMGELEVATOR : FacepunchBehaviour // credits: bmgjet
        {
            ...
            public static BMGELEVATOR FixElevator(EldrunRaidBases instance, List<BaseEntity> elevators)
            {
            ...
            {
                if (options.RequiredAccessLevel == 0 || _granted.Contains(player.userID) || player.HasPermission("eldrunraidbases.elevators.bypass.card"))
                {
                    return true;
                }
            ...
            {
                if (!options.RequiresBuildingPermission || player.HasPermission("eldrunraidbases.elevators.bypass.building") || raid.priv.IsKilled() || raid.priv.IsAuthed(player))
                {
                    return true;
                }
        ...
        public class RaidableSpawns
        {
            ...
            public RaidableSpawns(EldrunRaidBases instance, HashSet<RaidableSpawnLocation> spawns)
            {
                Spawns = spawns;
                Instance = instance;
                IsCustomSpawn = true;
            ...
            public RaidableSpawns(EldrunRaidBases instance)
            {
                Instance = instance;
            }
        ...
        public class PlayerInputEx : FacepunchBehaviour
        {
            ...
            private EldrunRaidBases Instance;
        ...
        public class RaidableBase : FacepunchBehaviour
        {
            ...
            public bool CanDropRustBackpack(ulong userid)
            {
                ...
                {
                    return !userid.HasPermission("eldrunraidbases.keepbackpackrust") && raiders.TryGetValue(userid, out var ri);
                }
                return false;
            }
            ...
            public bool CanDropBackpack(ulong userid)
            {
                ...
                {
                    return !userid.HasPermission("eldrunraidbases.keepbackpackplugin") && raiders.TryGetValue(userid, out var ri);
                }
                return false;
            }
            ...
            {
                if (Instance.FauxAdmin != null && player.IsNetworked() && player.IsDeveloper && player.HasPermission("fauxadmin.allowed") && player.HasPermission("eldrunraidbases.block.fauxadmin") && player.IsCheating())
                {
            ...
            {
                if (player.HasPermission("eldrunraidbases.banned"))
                {
            ...
            {
                if (!config.Settings.Management.AllowTeleport && !TeleportExceptions.Contains(player.userID) && player.IsConnected && !CanBypass(player) && NearFoundation(player.transform.position) && !IsMounted(player) && Interface.CallHook("OnBlockEldrunRaidBasesTeleport", player, Location) == null)
                {
            ...
            {
                if (!player.IsNetworked() || CanBypass(player) || player.HasPermission("eldrunraidbases.hoggingbypass"))
                {
                    return false;
                }
                ...
                if (!config.Settings.Management.IsBlocking() || player.HasPermission("eldrunraidbases.blockbypass"))
                {
                    return false;
                }
            ...
            {
                if (userid.HasPermission("eldrunraidbases.notitle"))
                {
                    return;
                }
            ...
            {
                return !player.IsHuman() || player.IsFlying || player.limitNetworking || player.HasPermission("eldrunraidbases.canbypass");
            }
            ...
            private bool CanBypassAuthorized(ulong userid) => userid.BelongsToGroup("admin") || userid.HasPermission("eldrunraidbases.canbypass");
            ...
            private void Subscribe()
            {
                ...
                if (Options.EnforceDurability && !Instance.permission.GroupHasPermission("default", "eldrunraidbases.durabilitybypass"))
                {
            ...
            private static void TakeLootFrom(EldrunRaidBases env, List<LootItem> lootList, List<LootItem> to, BuildingOptions Options, ulong UserID, bool AllowPVP)
            {
                if (lootList.Count == 0)
                {
                    return;
            ...
            public void ToggleLights()
            {
                ...
                if (ovens?.Count > 0)
                {
                    foreach (var oven in ovens)
                    {
                        ...
                        if (!state && (oven.ShortPrefabName.Contains("furnace") && oven.IsInvoking(oven.Cook))) continue;
        ...
        public class SpawnsControllerManager
        {
            ...
            private bool IsValidLocation(Vector3 v, float safeRadius, float minProtectionRadius, float railRadius)
            {
                ...
                return TopologyChecks(null, v, minProtectionRadius, railRadius, out var topology);
            }

            internal bool TopologyChecks(ManagementBiomeSettings biomes, Vector3 v, float radius, float railRadius, out string topology)
            {
                if (biomes != null && !biomes.IsBiomeEnabled(v, out var biome))
                {
        ...
        {
            if (player.IsAlive() && player.HasPermission("eldrunraidbases.mapteleport") && !player.isMounted)
            {
        ...
        private void OnServerInitialized(bool isStartup)
        {
            ...
            EldrunRaidBasesExtensionMethods.ExtensionMethods._permission ??= permission; 
            ...
            AddCovalenceCommand("rb.config", nameof(CommandConfig), "eldrunraidbases.config");
            AddCovalenceCommand("rb.populate", nameof(CommandPopulate), "eldrunraidbases.config");
            AddCovalenceCommand("rb.toggle", nameof(CommandToggle), "eldrunraidbases.config");
            LoadPlayerData();
            InitializeSkins();
            Initialize();
        ...
        private object OnNeverWear(Item item, float amount)
        {
            ...
            if (player == null || !player.IsHuman() || player.HasPermission("eldrunraidbases.durabilitybypass"))
            {
                return null;
            }
        ...
        private void OnLoseCondition(Item item, ref float amount)
        {
            ...
            if (player == null || !player.userID.IsSteamId() || player.HasPermission("eldrunraidbases.durabilitybypass"))
            {
                return;
            }
        ...
        {
            if (!player.IsKilled() && !player.HasPermission("eldrunraidbases.allow.commands"))
            {
        ...
        private void OnEntitySpawned(PlayerCorpse corpse)
        {
            ...
            else if (corpse.skinID == 14922524)
            {
        ...
        private void CommandRaidHunter(IPlayer user, string command, string[] args)
        {
            ...
            switch (arg)
            {
                ...
                case "version":
                    {
                        Message(user, $"EldrunRaidBases {Version} by nivex");
                        return;
                    }
                case "unban":
                    {
                        ...
                        void Revoke(string userid)
                        {
                            ...
                            {
                                if (permission.GroupHasPermission(group, "eldrunraidbases.banned"))
                                {
                                    permission.RevokeGroupPermission(group, "eldrunraidbases.banned");
                                    user.Message($"Banned permission has been removed from group: {group}");
                                }
                            }
                            if (permission.UserHasPermission(userid, "eldrunraidbases.banned"))
                            {
                                permission.RevokeUserPermission(userid, "eldrunraidbases.banned");
                ...
                case "savefix":
                    {
                        if (user.IsAdmin || user.HasPermission("eldrunraidbases.allow"))
                        {
                ...
                case "tp":
                    {
                        if (player.IsNetworked() && (isAdmin || user.HasPermission("eldrunraidbases.allow")))
                        {
                ...
                case "grid":
                    {
                        if (player.IsNetworked() && (isAdmin || user.HasPermission("eldrunraidbases.ddraw")))
                        {
            ...
            if (player.IsNetworked())
            {
                DrawRaidLocations(player, isAdmin || player.HasPermission("eldrunraidbases.ddraw"));
            }
        }
        ...
        private void CommandInvite(IPlayer user, BasePlayer player, string[] args)
        {
            ...
            if (user.IsServer || player.IsAdmin || user.HasPermission("eldrunraidbases.allow")) Message(user, $"You can use this command to set them as the owner of this raid: {config.Settings.EventCommand} setowner {target.userID}");
            raider.IsAlly = true;
        ...
        {
            if (!player.HasPermission("eldrunraidbases.block.filenames") && !player.IsAdmin && !player.IsDeveloper)
            {
                foreach (var raid in Raids)
                {
        ...
        {
            var player = user.Player();
            bool isAllowed = user.IsServer || player.IsAdmin || user.HasPermission("eldrunraidbases.allow");
        ...
        private bool CanCommandContinue(BasePlayer player, IPlayer user, bool isAllowed, string[] args)
        {
            ...
            if (!(user.IsServer || player.IsAdmin || user.HasPermission("eldrunraidbases.bypassmaxmanualeventlimit")) && Get(RaidableType.Manual) >= config.Settings.Manual.Max)
            {
        ...
        private bool HandledCommandArguments(BasePlayer player, IPlayer user, bool isAllowed, string[] args)
        {
            ...
            switch (args[0].ToLower())
            {
                ...
                case "lockraid":
                    {
                        if (args.Length >= 2 && (isAllowed || user.HasPermission("eldrunraidbases.setowner")))
                        {
                ...
                case "clearowner":
                    {
                        if (player.IsNetworked() && (isAllowed || user.HasPermission("eldrunraidbases.clearowner")))
                        {
                            var target = player;
        ...
        {
            if (isAllowed || player.HasPermission("eldrunraidbases.ddraw"))
            {
                AdminCommand(player, () =>
                {
                    foreach (var raid in Raids)
        ...
        {
            if (!user.HasPermission("eldrunraidbases.config"))
            {
                return;
            }
        ...
        private void CommandPopulate(IPlayer user, string command, string[] args)
        {
            ...
            user.Reply("Created oxide/data/EldrunRaidBases/Editable_Lists/Normal.json");

            AddToList("Random", items);
            user.Reply("Created oxide/data/EldrunRaidBases/Editable_Lists/Default.json");
        ...
        {
            permission.RegisterPermission("eldrunraidbases.allow", this);
            permission.RegisterPermission("eldrunraidbases.allow.commands", this);
            permission.RegisterPermission("eldrunraidbases.bypassmaxmanualeventlimit", this);
            permission.RegisterPermission("eldrunraidbases.setowner", this);
            permission.RegisterPermission("eldrunraidbases.clearowner", this);
            permission.RegisterPermission("eldrunraidbases.ladder.exclude", this);
            permission.RegisterPermission("eldrunraidbases.durabilitybypass", this);
            permission.RegisterPermission("eldrunraidbases.ddraw", this);
            permission.RegisterPermission("eldrunraidbases.mapteleport", this);
            permission.RegisterPermission("eldrunraidbases.canbypass", this);
            permission.RegisterPermission("eldrunraidbases.blockbypass", this);
            permission.RegisterPermission("eldrunraidbases.banned", this);
            permission.RegisterPermission("eldrunraidbases.vipcooldown", this);
            permission.RegisterPermission("eldrunraidbases.notitle", this);
            permission.RegisterPermission("eldrunraidbases.block.fauxadmin", this);
            permission.RegisterPermission("eldrunraidbases.elevators.bypass.building", this);
            permission.RegisterPermission("eldrunraidbases.elevators.bypass.card", this);
            permission.RegisterPermission("eldrunraidbases.hoggingbypass", this);
            permission.RegisterPermission("eldrunraidbases.block.filenames", this);
            permission.RegisterPermission("eldrunraidbases.keepbackpackplugin", this);
            permission.RegisterPermission("eldrunraidbases.keepbackpackrust", this);
        }
        ...
        private bool AssignTreasureHunters()
        {
            ...
            foreach (var target in covalence.Players.All)
            {
                if (target.Id.HasPermission("eldrunraidbases.th"))
                {
                    permission.RevokeUserPermission(target.Id, "eldrunraidbases.th");
                }
        ...
        {
            if (userid.HasPermission("eldrunraidbases.notitle") || userid.HasPermission("eldrunraidbases.ladder.exclude"))
            {
                return false;
            }
        ...
        private void AssignTreasureHunters(List<KeyValuePair<string, PlayerInfo>> players)
        {
            ...
            foreach (var kvp in k)
            {
                ...
                if (p == null || p.HasPermission("eldrunraidbases.notitle") || p.HasPermission("eldrunraidbases.ladder.exclude"))
                {
                    continue;
                }

                permission.GrantUserPermission(p.Id, "eldrunraidbases.th", this);
                ...
                Puts(mx("Log Granted", null, p.Name, p.Id, "eldrunraidbases.th", "raidhunter"));
            }
        }
        ...
        protected override void LoadDefaultMessages()
        {
            if (!isDefaultMessagesLoaded)
            {
                lang.RegisterMessages(new()
                {
                    ...
                    ["BannedAdmin"] = "You have the eldrunraidbases.banned permission and as a result are banned from these events.",
        ...
        public class ManagementBiomeSettings
        {
            ...
            public bool IsBiomeEnabled(Vector3 a, out TerrainBiome.Enum biome)
            {
                int? t = TerrainMeta.BiomeMap?.GetBiomeMaxType(a);
                if (!t.HasValue)
                {
        ...
        {
            [JsonProperty(PropertyName = "Wipe Management (/data/EldrunRaidBases.json)")]
        ...
        protected override void LoadConfig()
        {
            ...
            try
            {
                ...
                {
                    Puts("STOP! You are trying to install the FREE version of the plugin!");
        ...
        public class UI
        {
            ...
            private static void ShowDelayUI(EldrunRaidBases Instance, BasePlayer player)
            {
                if (player.IsKilled())
                {
                    return;
            ...
            public static void UpdateDelayUI(EldrunRaidBases Instance, BasePlayer player)
            {
            ...
            private static void SetDelayUpdate(EldrunRaidBases Instance, BasePlayer player)
            {
            ...
            public static bool ShowStatusUi(EldrunRaidBases Instance, BasePlayer player)
            {
                float radius = 5f;
            ...
            public static void UpdateStatusUI(EldrunRaidBases Instance, BasePlayer player)
            {
            ...
            private static void SetStatusUpdate(EldrunRaidBases Instance, BasePlayer player)
            {
                float radius = 5f;
...
namespace Oxide.Plugins.EldrunRaidBasesExtensionMethods
{
