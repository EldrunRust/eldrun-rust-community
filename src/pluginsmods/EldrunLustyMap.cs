...
{
    [Info("EldrunLustyMap", "Eldrun", "36187")]
    [Description("In-game map and minimap GUI")]
    class EldrunLustyMap : RustPlugin
    {
        #region Fields
        ...
        static EldrunLustyMap instance;
        ...
        void Loaded()
        {
            ...
            permission.RegisterPermission("eldrunlustymap.admin", this);
        }
        ...
        void OnEntityKill(BaseNetworkable entity)
        {
            ...
            if (entityMarkers.ContainsKey((uint)entity.net.ID.Value))
                entityMarkers.Remove((uint)entity.net.ID.Value);
        ...
        {
            if (ImageLibrary?.Call<bool>("IsReady") != true)
            {
        ...
        void CreateStaticMain()
        {
            ...
            if (ImageLibrary?.Call<bool>("HasImage", "mapimage", 0UL) == true)
                mapimage = GetImage("mapimage");
            else if (ImageLibrary?.Call<bool>("HasImage", "mapimage_high", 0UL) == true)
        ...
        void cmdOpenMap(BasePlayer player, string command, string[] args)
        {
            ...
            else
            {
                ...
                {
                    if (!player.IsAdmin && !permission.UserHasPermission(player.UserIDString, "eldrunlustymap.admin")) return;
                    if (user.IsAdmin)
                    {
        ...
        private void AddTemporaryEntityMarker(BaseEntity entity)
        {
            ...
            entityMarkers.Add((uint)entity.net.ID.Value, actEnt);
        }
        ...
        bool AddMarker(float x, float z, string name, string icon = "special", float r = 0)
        {
            ...
            if (!string.IsNullOrEmpty(icon) && icon != "special")
            {
                ...
                ImageLibrary?.Call("AddImage", url, name, 0UL);
            }
            SaveMarkers();
            return true;
        }
        void UpdateMarker(float x, float z, string name, string icon = "special", float r = 0)
        {
            ...
            if (!string.IsNullOrEmpty(icon) && icon != "special" && ImageLibrary?.Call<bool>("HasImage", name, 0UL) != true)
            {
                string url = icon;
        ...
        private string GetImage(string name)
        {
            ...
            return ImageLibrary?.Call<string>("GetImage", name, 0UL);
        }
        void ValidateImages()
        {
            Puts("[Warning] Validating imagery");
            if (isNewSave || ImageLibrary?.Call<bool>("HasImage", "mapimage", 0UL) != true)
            {
                LoadImages();
                LoadMapImage();
            }
        ...
        private void LoadImages()
        {
            ...
            ImageLibrary?.Call("ImportImageList", Title, newLoadOrder, 0UL, true);
        }
        private void LoadMapImage()
        {
            if (configData.Map.MapImage.CustomMap_Use)
            {
                ...
                ImageLibrary?.Call("AddImage", dataDirectory + configData.Map.MapImage.CustomMap_Filename, "mapimage_high", 0UL);
                ScaleMapImage();
                if (MapSettings.complexmap)
                {
        ...
        void DownloadMap(string url)
        {
            ...
            ImageLibrary?.Call("AddImage", url, "mapimage_high", 0UL);
            ScaleMapImage();
        ...
        void ScaleMapImage()
        {
            if (ImageLibrary?.Call<bool>("HasImage", "mapimage_high", 0UL) == true)
            {
                ...
                ImageLibrary?.Call("ImportImageData", $"{instance.Title} - Map Image", new Dictionary<string, byte[]> { { "mapimage", bytes } }, 0UL, true);
            }
        ...
        class MapSplitter
        {
            public bool SplitMap(string imageId, int amount)
            {
                ...
                instance.ImageLibrary?.Call("ImportImageData", $"{instance.Title} - Complex ({amount})", newLoadOrder, 0UL, true);
                return true;
            }
