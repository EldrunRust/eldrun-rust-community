...
namespace Oxide.Plugins
{
    ...
    class EldrunCoinFlip : CovalencePlugin
    {
        ...
        // Permissions
        private const string PERMISSION_CREATE = "eldruncoinflip.create";
        private const string PERMISSION_JOIN = "eldruncoinflip.join";
        #endregion

        #region Data
        // Configuration values
        private int MinimumAmount => (int)Config[EldrunCoinFlip.CONFIG_MINIMUM_AMOUNT];
        private int Timeout => (int)Config[EldrunCoinFlip.CONFIG_TIMEOUT];
        private bool AnnounceEverybody => (bool)Config[EldrunCoinFlip.CONFIG_ANNOUNCE_EVERYBODY];
        private string Prefix => (string)Config[EldrunCoinFlip.CONFIG_PREFIX];
        ...
        protected override void LoadDefaultConfig()
        {
            ...
            Config[EldrunCoinFlip.CONFIG_MINIMUM_AMOUNT] = 0;
            Config[EldrunCoinFlip.CONFIG_TIMEOUT] = 5;
            Config[EldrunCoinFlip.CONFIG_ANNOUNCE_EVERYBODY] = true;
            Config[EldrunCoinFlip.CONFIG_PREFIX] = "[Coin Flip]";
        }
        ...
        #region Commands
        [Command("coinflip"), Permission(EldrunCoinFlip.PERMISSION_CREATE)]
        private void coinFlipChatCommand(IPlayer player, string command, string[] args)
        {
            ...
            if (this.sendConditionalLocalisedMessage(player, targetPlayer != null && !targetPlayer.HasPermission(EldrunCoinFlip.PERMISSION_JOIN), Messages.CANT_PARTICIPATE, targetPlayer?.Name))
                return;
            ...
            if (targetPlayer == null)
            {
                ...
                this.covalence.Players.Connected.ToArray().Where(p => p.Id != player.Id && p.HasPermission(EldrunCoinFlip.PERMISSION_JOIN)).ToList().ForEach(p => this.sendLocalisedMessage(p, Messages.COINFLIP_INITIATED, player.Name, amount));
        ...
        [Command("joinflip"), Permission(EldrunCoinFlip.PERMISSION_JOIN)]
        ...
        [Command("cancelflip"), Permission(EldrunCoinFlip.PERMISSION_CREATE)]
        ...
        private void flip(IPlayer initiator, IPlayer player, CoinFlipParams coinFlipParams)
        {
            ...
            if (EldrunCoinFlip.RNG.Next(0, 2) == 1)
            {
                winner = initiator;
                loser = player;
            }
