...
using Oxide.Plugins.EldrunBetterLootExtenstions;
using System;
...
{
    [Info("EldrunBetterLoot", "Eldrun", "36187")]
    ...
    public class EldrunBetterLoot : RustPlugin
    {
        #region Fields
        [PluginReference] Plugin? CustomLootSpawns;

        // Static Instance
        private static EldrunBetterLoot? _instance;
        ...
        private const string ADMIN_PERM = "eldrunbetterloot.admin";
        #endregion
        ...
        private class ChatConfiguration
        {
            ...
            public string prefix = $"[<color=#00ff00>{nameof(EldrunBetterLoot)}</color>]";
        ...
        {
            Log($"Configuration changes saved to {nameof(EldrunBetterLoot)}.json");
        ...
        private static class DataSystem
        {
            ...
            public static void BakDataFile(string filename, bool restoreMode = false, BasePlayer? msgPlayer = null)
            {
                ...
                string path = Path.Combine(Interface.Oxide.DataFileSystem.Directory, $"{nameof(EldrunBetterLoot)}/{filename}.json");
            ...
            private static void LoadFile<T>(string fileName, Func<T, T>? validator, ref T loadVar, Action<T>? postLoadMethod = null) where T : class?
            {
                ...
                try
                {
                    loadVar = validator(Interface.Oxide.DataFileSystem.ReadObject<T>($"{nameof(EldrunBetterLoot)}\\{fileName}"));
            ...
            private static void SaveFile<T>(string fileName, Func<T, T>? validator, ref T saveVar) where T : class?
            {
                ...
                Interface.Oxide.DataFileSystem.WriteObject($"{nameof(EldrunBetterLoot)}\\{fileName}", validator(saveVar));
...
namespace Oxide.Plugins.EldrunBetterLootExtenstions
{
