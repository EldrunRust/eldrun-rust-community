...
class EldrunDLCAPI : RustPlugin
{
    ...
    private void CheckSteamDefinitionsUpdated()
    {
        ...
        {
            if (timeWaited % 10f == 0)
                Debug.LogWarning($"[EldrunDLCAPI] - Waiting for Steam inventory definitions to be updated. Total wait time : {Math.Round(time, 1)}s");
