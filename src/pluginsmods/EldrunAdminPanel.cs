...
{
    [Info("EldrunAdminPanel", "Eldrun", "36187")]
    [Description("GUI admin panel with command buttons")]
    class EldrunAdminPanel : RustPlugin
    {
        [PluginReference]
        private Plugin AdminRadar, Godmode, Vanish, PlayerAdministration;

        private const string permAdminPanel = "eldrunadminpanel.allowed";
        ...
        private const string permAutoToggle = "eldrunadminpanel.autotoggle.admin";
        ...
        [ConsoleCommand("eldrunadminpanel")]
        private void ccmdAdminPanel(ConsoleSystem.Arg arg)
        {
            ...
            switch (arg.Args[0].ToLower())
            {
                        ...
                        if (arg.Args.Length >= 2)
                        {
                            switch (arg.Args[1].ToLower())
                            {
                                ...
                                case "newtp":
                                    ...
                                    break;
                                default:
                                    arg.ReplyWith(_("Syntax", player.UserIDString, "eldrunadminpanel", "action vanish/admintp/radar/god/newtp"));
                                    break;
                            }
                        }
                        else
                ...
                default:
                    {
                        arg.ReplyWith(_("Syntax", player.UserIDString, "eldrunadminpanel", "action/toggle"));
                        break;
                    }
            }
        }

        [ChatCommand("eldrunadminpanel")]
            ...
            NextTick(() =>
            {
                ...
                if (AdminRadar != null && permission.UserHasPermission(player.UserIDString, permAdminRadar))
                {
                    GUIElement.Add(new CuiButton
                    {
                        Button =
                        {
                            Command = "eldrunadminpanel action radar",
                            Color = BTNColorRadar
                        },
                        Text =
                        {
                ...
                if (PlayerAdministration != null && permission.UserHasPermission(player.UserIDString, permPlayerAdministration))
                {
                    GUIElement.Add(new CuiButton
                    {
                        Button =
                        {
                            Command = "eldrunadminpanel action pa",
                            Color = BTNColorPA
                        },
                        Text =
                        {
                ...
                GUIElement.Add(new CuiButton
                {
                    Button =
                    {
                        Command = "eldrunadminpanel action admintp",
                ...
                if (config.newtp.enabled)
                {
                    GUIElement.Add(new CuiButton
                    {
                        Button =
                        {
                            Command = "eldrunadminpanel action newtp",
                            Color = BTNColorNewTP
                        },
                        Text =
                        {
                ...
                if (Godmode != null && permission.UserHasPermission(player.UserIDString, permGodmode))
                {
                    GUIElement.Add(new CuiButton
                    {
                        Button =
                        {
                            Command = "eldrunadminpanel action god",
                            Color = BTNColorGod
                        },
                        Text =
                        {
                ...
                if (Vanish != null && permission.UserHasPermission(player.UserIDString, permVanish))
                {
                    GUIElement.Add(new CuiButton
                    {
                        Button =
                        {
                            Command = "eldrunadminpanel action vanish",
